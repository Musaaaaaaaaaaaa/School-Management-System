...
function Expenses() {
  ...
  const manualExpenseData = [
    {
      entry: 'salary',
      monthly: ['Rs 456', 'Rs 456', 'Rs 956', 'Rs 426', 'Rs 256', 'Rs 411', 'Rs 456', 'Rs 456', 'Rs 956', 'Rs 426', 'Rs 256', 'Rs 411']
    },
    {
      entry: 'furniture',
      monthly: ['Rs 456', 'Rs 456', 'Rs 956', 'Rs 426', 'Rs 256', 'Rs 411', 'Rs 456', 'Rs 456', 'Rs 956', 'Rs 426', 'Rs 256', 'Rs 411']
    ...
    datasets: [
      {
        data: expenseTypes.map(expenseTypeIndex =>
          budgets[expenseTypeIndex]?.reduce((acc, budget) => acc + (parseFloat(budget) || 0), 0) || 0
        ),
      },
    ],
  };
    ...
    <View style={styles.container}>
      ...
      {displayTable && (
        <ScrollView contentContainerStyle={styles.scrollViewContent}>
          ...
          {/* Manually Display SQL Backend Data */}
          <View style={{ margin: 20, borderWidth: 1, borderColor: '#000', borderRadius: 10, backgroundColor: 'white' }}>
            <Text style={{ fontSize: 20, fontWeight: 'bold', textAlign: 'center', marginVertical: 10 }}>Backend Expenses (Static)</Text>
            <ScrollView horizontal>
              <DataTable>
                <DataTable.Header>
                  <DataTable.Title style={{ minWidth: 100 }}>Entry</DataTable.Title>
                  {months.map((month, index) => (
                    <DataTable.Title key={index} style={{ minWidth: 100 }}>{month.label.slice(0, 3)}</DataTable.Title>
                  ))}
                </DataTable.Header>
                {manualExpenseData.map((row, rowIndex) => (
                  <DataTable.Row key={rowIndex}>
                    <DataTable.Cell style={{ minWidth: 100 }}>{row.entry}</DataTable.Cell>
                    {row.monthly.map((val, colIndex) => (
                      <DataTable.Cell key={colIndex} style={{ minWidth: 100 }}>{val}</DataTable.Cell>
                    ))}
                  </DataTable.Row>
                ))}
              </DataTable>
